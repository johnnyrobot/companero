...
const VERIFY_PROMPT = (claim, v) =>
  ...
  "\n\n[Re-verification pass: the initial automated cross-check was aborted by a transient upstream rate limit before any votes were cast. Adjudicate this claim fresh, on its merits.]"
...
phase("Verify")
// Re-run in smaller sequential waves so we don't re-trip the upstream rate
// limit that aborted the first verification pass (all 75 web-tool agents fired
// at once). ~6 claims/wave keeps ≤18 verify agents in flight per wave.
const VERIFY_WAVE = 6
const verifyClaim = claim => () =>
  parallel(
...
const voted = []
for (let i = 0; i < rankedClaims.length; i += VERIFY_WAVE) {
  const wave = rankedClaims.slice(i, i + VERIFY_WAVE)
  log("Verify wave " + (Math.floor(i / VERIFY_WAVE) + 1) + "/" + Math.ceil(rankedClaims.length / VERIFY_WAVE) + " — " + wave.length + " claims")
  const waveVoted = (await parallel(wave.map(verifyClaim))).filter(Boolean)
  voted.push(...waveVoted)
}
